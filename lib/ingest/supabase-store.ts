...
import { DatasetInbox, attachmentIdentity, sha256, type AttachmentRecord, type EmailRecord, type IngestStore, MissingAttachmentError } from "./dataset";
...
export class SupabaseIngestStore implements IngestStore {
  ...
  async email(row: EmailRecord) {
    const existing = await this.db.from("emails").select("sender,subject,body,source").eq("id", row.id).maybeSingle();
    check(existing.error, "Read existing email");
    if (existing.data && (existing.data.source !== "dataset" || (["sender","subject","body"] as const).some(k => existing.data![k] !== row[k]))) {
      throw new Error(`Email ${row.id} already exists with different content. Import revisions through the future revision workflow.`);
    }
    check((await this.db.from("emails").upsert(row, { onConflict: "id" })).error, "Upsert email");
  }
  async attachment(row: AttachmentRecord, bytes: Uint8Array | null) {
    const existing = await this.db.from("attachments").select("content_sha256,ingest_state").eq("id", row.id).maybeSingle();
  ...
  async audit(emailId: string, detail: Record<string, unknown>) {
    check((await this.db.from("audit_log").insert({ email_id: emailId, actor: "system", action: "dataset_ingested", detail })).error, "Write ingestion audit");
  }
}
...
export async function verifySupabaseSeed(db: SupabaseClient, inbox: DatasetInbox) {
  ...
  for (const email of emails) {
    ...
    const rows = await db.from("attachments").select("*").eq("email_id", email.email_id);
    check(rows.error, "Verify attachment rows");
    if (rows.data!.length !== email.attachments.length) throw new Error(`Attachment row count mismatch: ${email.email_id}`);
    for (const sourcePath of email.attachments) {
      const identity = attachmentIdentity(email.email_id, sourcePath);
      const row = rows.data!.find(r => r.id === identity.id && r.source_path === sourcePath);
      if (!row) throw new Error(`Attachment reference missing: ${email.email_id}/${sourcePath}`);
      let source: Uint8Array;
      try { source = await inbox.read_bytes(sourcePath); }
      catch (error) {
        if (!(error instanceof MissingAttachmentError)) throw error;
        if (row.ingest_state !== "missing") throw new Error("Missing source not recorded.");
        missingAttachments++; verifiedAttachments++; continue;
      }
      if (row.ingest_state !== "stored" || row.content_sha256 !== sha256(source)) throw new Error(`Attachment metadata mismatch: ${sourcePath}`);
