...
const labels: Record<ScoredField, string[]> = {
  ...
  container_count: ["no. of containers or packages", "no of containers", "number of containers", "container count", "containers", "no. of cntrs", "qty of containers"],
  gross_weight_kg: ["gross weight (kg)", "gross weight", "gross wt (kgs)", "gross wt", "g.w."],
};
function esc(value: string) { return value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"); }
function missing(value: string) { return !value.trim() || /^(?:n\/?a|tba|_+|-+)$/i.test(value.trim()); }
export function extractLexicon(lines: Line[]): ExtractedValue[] {
  return scoredFields.map(field => {
    const matches = labels[field].map(label => new RegExp(`^\\s*${esc(label)}\\s*(?:[:\-]|\\|)?\\s*(.*)$`, "i"));
    ...
    return { field, rawValue: null, normalizedValue: null, evidenceSnippet: null, evidenceLine: null, extractedBy: "lexicon" };
  });
}
