...
async function main() {
      ...
      JSON.stringify(
        {
          mode: "dry-run",
          total: emails.length,
          ruleCounts: Object.fromEntries(counts),
          llmFallbackCandidates: llmNeeded,
        },
        null,
        2,
      ),
  ...
  if (values["retry-failures"]) {
    const failed = await db
      .from("emails")
      .select("id")
      .eq("category_confidence", 0);
    if (failed.error)
      throw new Error(`Find failed classifications: ${failed.error.message}`);
    const ids = new Set(failed.data.map((row) => row.id));
    targets = emails.filter((email) => ids.has(email.email_id));
  }
  console.log(
    `Classifying ${targets.length} emails. Gemini is called only for low-confidence messages.`,
  );
  for (let index = 0; index < targets.length; index++) {
    ...
    try {
      ...
      result = {
        ...
        reasoning: "LLM classification failed; defaulted safely to GENERAL.",
      };
      const detail = {
        message:
    ...
    if (index === 0 || (index + 1) % 10 === 0 || index + 1 === targets.length)
      console.log(`Classified ${index + 1}/${targets.length}`);
  }
  console.log(
    JSON.stringify(
      {
        mode: values["retry-failures"] ? "retry-failures" : "complete",
