...
async function main() {
  ...
  for (let index = 0; index < emails.length; index++) {
    ...
    const parsed: Array<{
      id: string;
      type: "SI" | "BL" | "unknown";
      fields: ReturnType<typeof extractLexicon>;
    }> = [];
    if (!reason)
      for (const attachment of attachments) {
        ...
        try {
          const document = await parseAttachment(
            attachment.filename,
          ...
          await db
            ...
            .update({
              ...
              is_readable: document.readable,
              read_method: document.method,
              doc_type: type,
            })
            .eq("id", attachment.id);
          ...
          const fields = extractLexicon(document.lines).map((field) => ({
            ...field,
            normalizedValue: normalize(field.rawValue, field.field),
          }));
          ...
          const extracted = fields.map((field) => ({
            ...
            evidence_snippet: field.evidenceSnippet,
            evidence_line: field.evidenceLine,
            extracted_by: field.extractedBy,
          }));
          const upsert = await db
            .from("extracted_fields")
            .upsert(extracted, { onConflict: "attachment_id,field_name" });
    ...
    const si = parsed.find((item) => item.type === "SI");
    const bl = parsed.find((item) => item.type === "BL");
    if (!reason && (!si || !bl))
      reason =
...
main().catch((error) => {
  console.error(error instanceof Error ? error.message : "Processing failed.");
  process.exitCode = 1;
});
